...
func TestPRMCacheRefresh(t *testing.T) {
	...
	cache.RegisterRequests([]model.PRMRequest{{DeviceUUID: "GPU-1", Port: 1, Group: 34}})
...
func TestPRMCacheRefreshPartialError(t *testing.T) {
	...
	cache := NewPRMCache()
	cache.client = sysprobeclient.GetCheckClient(sysprobeclient.WithSocketPath(socketPath))
	cache.RegisterRequests([]model.PRMRequest{
		{DeviceUUID: "GPU-1", Port: 1, Group: 34},
		{DeviceUUID: "GPU-1", Port: 2, Group: 34},
	})
