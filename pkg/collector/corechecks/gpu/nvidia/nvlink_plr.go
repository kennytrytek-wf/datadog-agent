...
func newNVLinkPLRCollector(device ddnvml.Device, port int, _ *CollectorDependencies) (Collector, error) {
...
func (c *nvlinkPLRCollector) Collect() ([]Metric, error) {
	...
	counters, err := c.readPortCounters(c.port)
	if err != nil {
		return nil, fmt.Errorf("read PLR counters for port %d: %w", c.port, err)
	}

	for _, field := range plrCounterFields {
		value, found := counters[field]
		if !found {
			return nil, fmt.Errorf("missing PLR counter %q for port %d", field, c.port)
		}
